...
#ifndef __BOX2D_CPP__
#define __BOX2D_CPP__
...
void SimulationLoop( HDC hdc )
{
	...
	/*TextOut( hdc, 5, 75, demoStrings[demoIndex], wcslen(demoStrings[demoIndex]) );
	DrawText(5, 15, demoStrings[demoIndex]);
	DrawText(5, 45, "Keys: 1-9 Demos, Space to Launch the Bomb");*/

	wchar_t buffer[64];
	wsprintf(buffer, L"(A)ccumulation %s", World::accumulateImpulses ? L"ON" : L"OFF");
	TextOut( hdc, 5, 75, buffer, wcslen(buffer) );
	//DrawText(5, 75, buffer);

	wsprintf(buffer, L"(P)osition Correction %s", World::positionCorrection ? L"ON" : L"OFF");
	TextOut( hdc, 5, 105, buffer, wcslen(buffer) );
	//DrawText(5, 105, buffer);

	wsprintf(buffer, L"(W)arm Starting %s", World::warmStarting ? L"ON" : L"OFF");
	TextOut( hdc, 5, 135, buffer, wcslen(buffer) );
	...
	wsprintf(buffer, L"%d", (int)&world);
	TextOut( hdc, 5, 165, buffer, wcslen(buffer) );

	for (int i = 0; i < numBodies; ++i)
		DrawBody(bodies + i, hdc);

	for (int i = 0; i < numJoints; ++i)
		DrawJoint(joints + i, hdc);
